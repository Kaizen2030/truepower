...
export default function BlogCard({ post, featured = false }) {
  ...
  const plainTextBody = (post.body || post.content || "")
    .replace(/<[^>]+>/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  const excerpt =
    post.excerpt || plainTextBody.slice(0, featured ? 180 : 110) || "";
  const readTime = Math.max(
    1,
    Math.ceil((plainTextBody.split(" ").filter(Boolean).length || 80) / 180),
  );
  ...
  return (
    ...
    >
      <div
        className={`relative min-h-[220px] sm:min-h-[340px] ${featured ? "min-h-[280px] md:min-h-[480px]" : ""}`}
      >
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 p-4 sm:p-6">
          ...
          <h2
            className={`max-w-3xl font-display font-bold leading-tight text-white drop-shadow-md ${featured ? "text-lg sm:text-3xl lg:text-[2rem] line-clamp-3" : "text-[15px] sm:text-xl line-clamp-3"}`}
          >
            {post.title}
          </h2>
