...
function itemLines(item) {
  ...
  const description = String(item?.description || item?.product_name || "Item")
    .trim()
    .split(/\s+/)
    .slice(0, 3)
    .join(" ");
  const descriptionWidth = Math.max(8, RECEIPT_COLUMNS - right.length - 1);
  const safeDescription = description.slice(0, descriptionWidth);
  return [`${safeDescription.padEnd(descriptionWidth)} ${right}`];
}
