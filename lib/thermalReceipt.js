...
function itemLines(item) {
  const amount = money((Number(item?.qty) || 0) * (Number(item?.price) || 0));
  const amountWidth = Math.min(14, amount.length);
  const descriptionWidth = RECEIPT_COLUMNS - amountWidth - 1;
  const descriptions = wrap(item?.description || item?.product_name || "Item", descriptionWidth);

  return descriptions.map((description, index) =>
    index === 0
      ? `${description.padEnd(descriptionWidth)} ${amount.padStart(amountWidth)}`
      : `  ${description}`,
  );
}
